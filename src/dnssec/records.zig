...
pub const Error = name_mod.Error || error{ InvalidLength, InvalidBitmap, Truncated };
...
pub const TypeBitmapIterator = struct {
    ...
    pub fn contains(self: TypeBitmapIterator, rr_type: types.Type) Error!bool {
        var it = self;
        const needle = @intFromEnum(rr_type);
        while (try it.next()) |item| {
            const value = @intFromEnum(item);
...
test "type bitmap iteration and lookup" {
    ...
    const bitmap: TypeBitmapIterator = .{ .bytes = &data };
    try std.testing.expect(try bitmap.contains(.A));
    try std.testing.expect(try bitmap.contains(.RRSIG));
    try std.testing.expect(!(try bitmap.contains(.AAAA)));
}
